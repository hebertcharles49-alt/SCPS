...
func _build() -> void:
	...
	_head.add_theme_color_override("font_color", VKit.COL_VALUE)   # encre titre — l'or pâle d'époque sombre est illisible sur parchemin
	...
	_status = Label.new()
	_status.add_theme_color_override("font_color", VKit.COL_PARCH)
	...
	_cd_lbl = Label.new()
	_cd_lbl.add_theme_color_override("font_color", Color(0.62, 0.42, 0.10))   # ambre encre
	...
	# TRIM IN-GAME (2026-07-25) : le « conseil contextuel » (À suivre…/Rappel…) était une
	# phrase explicative permanente en pied de tiroir — retiré (doctrine : pas de prose).
	_context_hint = null
...
## marge, et sa hauteur HUG le contenu VISIBLE — au-delà du plafond, le ScrollContainer
## prend le relais. PIÈGE (2026-07-25, la régression « chunky ») : mesurer le min du VBox
## SYNCHRONEMENT après un rebuild ment — les labels autowrap pas encore layoutés (largeur
## 0) rapportent un mot par ligne (mesuré : 2638 px pour ~600 réels) → clamp au plafond →
## pleine hauteur permanente. Motif _fit_scroll (construction_panel) : largeur posée tout
## de suite, hauteur mesurée après DEUX frames de grâce, jeton anti-course.
func _layout() -> void:
	...
	await get_tree().process_frame
	if gen != _layout_gen or _panel == null or not is_instance_valid(_panel):
		return
	var content_h := 160.0
	...
	if OS.has_environment("SCPS_UIDBG"):
		print("[diplo_layout] content_h=%.0f ceiling=%.0f real=%.0f" % [content_h, ceiling, _panel.size.y])
...
func _update_action_detail(verb: String, legal: Dictionary, amber: bool) -> void:
	...
	# TRIM IN-GAME (2026-07-25, « une métrique, un mot, un chiffre ») : l'état en UN mot +
	# les chiffres ; le POURQUOI vit au hover (checklist ✓/✗ + ACTION_HELP en tooltip).
	...
	if not allowed:
		...
		state = "Refus probable"
		col = Color(0.62, 0.42, 0.10)   # ambre ENCRE (lisible sur parchemin, plus le pastel d'époque sombre)
	elif unilateral:
		state = "Unilatéral"
	else:
		state = "Acceptation probable"
