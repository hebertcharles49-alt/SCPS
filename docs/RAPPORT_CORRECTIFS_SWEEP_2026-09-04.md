...
Branche `claude/vibrant-euler-1tgfp3`, commits 37680a3 → vague A (2026-09-03/04). Save v109. Golden et golden-deep re-baselinés trois fois (fin W1 : 908a7cd ; fin W2 : 64146e4 ; fin vague A). Toutes les décisions joueur citées sont actées dans TROUVAILLES.md et la mémoire de session.
...
| 49a6c94 | Queue de levée & riche désarmé (A2) | La levée ne gatait jamais l'ARSENAL (l'éthos réclamait une catégorie absente → 0) ; le fer prélevé avant un recrutement tout-ou-rien ; frein désarmé quand le revenu fiscal lit 0 | `WH_ARSENAL_GATE`, `WH_POOL_CLAMP` (levée partielle), `WH_MILICE_FLOOR`, `WH_REV_FALLBACK` — aucun plafond | 250 ans apparié (s3) : armée/limite max 432 % → 313 %, solde/revenu max 3 981 % → 1 404 %, arsenal vide 3 063 → 0 pays-an, levées ×1,9 et désertions ×2,1 ; le riche désarmé (Ligue Dhûrganyn, s11) passe de 0 à 65 rgt ; plus aucun empire à 0 rgt avec or, armes et pas d'armée au front. Queue ATTÉNUÉE, pas fermée : les offenders restants ont 14-30 paquets au front qui ne coûtent rien → A4 (WH_PAY_CORPS) |
| 4d30ca8 | Queue de levée, le trou restant (A4) | Un régiment au front était GRATUIT : `typed_pay` n'itérait que le host, `campaign_order` transfère tout au corps ; la désertion ne mordait jamais sur le front | Solde et désertion sur host + corps (`WH_PAY_CORPS`, 3 `static inline` d'en-tête, corps vidé → prune), chronicle « solde X dont corps Y (Z %) » | Apparié 3 graines × 250 ans : armée/limite max 432 → 185 % (s3), 218 → 138 % (s60), 331 → 185 % (s11) ; solde/revenu max 3 981 → 1 175 %, 19 066 → 1 805 % ; désertions +7 à +76 % ET poste soldes plus BAS ; guerres et conquêtes intactes, 0 nul. s60 diverge sur une graine (satisfaction, figées) : trajectoire à re-juger en 3×3 |
| — | Marbrive (anomalie 5) | Faux constat de W2-6 (une graine, 60 ans) | rien : 63 déclenchements sur 27 sims | — |

Reste de la famille (décision joueur) : la LIMITE DE FORCE et l'intendance de dépassement restent calculées sur le host seul — un pays à 3 rgt de host et 200 rgt de corps ne paie aucune intendance.
