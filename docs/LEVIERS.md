...
| `PL_SINK_MONTHS` / `PL_SINK_FLOOR` | 0 / 0 | **A3 (2026-09-04) : plus AUCUNE réserve retranchée du trésor pour former la caisse de `price_level`.** La réserve dit la SOLVABILITÉ (`SINK_MONTHS`/`SINK_FLOOR`, qui bornent l'entretien et la redépense), jamais la valeur de la monnaie : soustraite du NUMÉRATEUR d'un niveau de prix, elle fabrique un prix NUL dès que le trésor passe dessous (graine 4243 an 118 : pays 54, revenu 14 704 or/an ⇒ réserve 3 676 > trésor 2 412 ⇒ ses 21 provinces à 0,00). Restaurer W2-1 : `PL_SINK_FLOOR=500 PL_SINK_MONTHS=3` · restaurer W1-A : `PL_SINK_FLOOR=500 PL_SINK_MONTHS=0` |
| `PL_EXPONENT` | 0.5 | **la VISCOSITÉ des prix** : dans quelle mesure le niveau de prix suit la monnaie d'État par unité de production RÉELLE. `1` = plein (théorie quantitative nue) · `0` = prix figés au pair (`price_level ≡ 1` : le prix n'est plus QUE l'offre/demande — kill-switch monétaire) · `0,5` = à moitié en log. Le défaut 0,5 n'est pas un confort : c'est le POINT FIXE de la formule d'avant (`pl = caisse/(Q×pl)` ⇒ `pl* = √(caisse/Q)`), l'échelle sur laquelle tout M3b/M12/M15 a été calibré à son insu. À `1,0`, graine 7 sur 120 ans : masse monétaire 1 664 602 → 398 825, manufactures privées 264 → 37, satisfaction Laborer 49 % → 34 % |
| `PL_LEGACY` | 0 | kill-switch MAÎTRE du niveau de prix : `1` = la formule d'avant A3, EXACTE (dénominateur = VA NOMINALE, réserve 500/3 mois, hameaux libres à `PL_GENESIS`). **Prouvé byte-identique** : `SCPS_TUNE=PL_LEGACY=1 make golden` → hash monde identique au golden commité |
