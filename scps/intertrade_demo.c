 ...
 * TRÉSOR NATIONAL (2026-09-03) : l'or d'État se somme au grain PAYS (nat_treasury) ;
 * la richesse des POPS reste provinciale, agrégée dans region[]. */
static double wsum(const WorldEconomy *e){
    double s=0.0;
    for(int c=0;c<SCPS_MAX_COUNTRY;c++) s += e->nat_treasury[c];
    for(int r=0;r<e->n_regions;r++)
        for (int c=0;c<CLASS_COUNT;c++) s += e->region[r].strata[c].wealth;
    return s;
}

/* LE COMMERCE PARLE EN RÉGIONS, L'OR ET LA MATIÈRE VIVENT EN NATIONS (2026-09-03,
 * docs/DESIGN_TRESOR_NATIONAL.md — mêmes accesseurs que it_stock/it_treasury côté moteur) :
 * « le stock/le trésor de cette région » = celui du PAYS qui la tient. Une région sans
 * maître n'a ni caisse ni entrepôt — les fixtures POSENT donc toujours un owner d'abord. */
static int rown(const WorldEconomy *e,int r){
    if(r<0||r>=e->n_regions) return -1;
    int o=e->region[r].owner; return (o>=0&&o<SCPS_MAX_COUNTRY)?o:-1;
}
static float rstock(const WorldEconomy *e,int r,int g){
    int o=rown(e,r); return (o<0)?0.f:e->nat_stock[o][g];
}
static void set_rstock(WorldEconomy *e,int r,int g,float v){
    int o=rown(e,r); if(o>=0) e->nat_stock[o][g]=v;
}
static double rgold(const WorldEconomy *e,int r){
    int o=rown(e,r); return (o<0)?0.0:econ_country_gold(e,o);
}
static void set_rgold(WorldEconomy *e,int r,float v){
    int o=rown(e,r); if(o>=0) e->nat_treasury[o]=v;
}
/* B9 (re-taillé 2026-09-03) : le stock et le trésor ne descendent PLUS à la province —
 * ils vivent au pays et ne peuvent donc plus être effacés par econ_aggregate_regions.
 * Il reste UNE chose à pousser vers prov[] : l'APPARTENANCE (econ_aggregate_regions ÉLIT
 * region[].owner depuis les provinces membres — un poke sur la seule vue serait annulé au
 * premier tick) et le PRIX de la représentative (que le marché relit). */
static void mirror_prov(WorldEconomy *e, int region){
    ...
    for (int p=0;p<n;p++){
        ...
        pv->owner = rv->owner;
        if (p==rep || rep<0)
            for (int g=0; g<RES_COUNT; g++) pv->price[g]=rv->price[g];
    }
}
...
int main(int argc,char**argv){
     ...
     * encore tourné) — wsum() lit la RICHESSE des classes sur region[] (l'or, lui, est
     * national et toujours à jour) et verrait sinon un état PÉRIMÉ/incohérent avec prov[]
     * (la vérité, seedée par gen_population), faussant le AVANT/APRÈS de la conservation. */
     ...
     * L'entrepôt et la caisse sont NATIONAUX : ra est au pays 0, rb au pays 1, et on pose
     * la dotation UNE fois par PAYS (set_rstock/set_rgold, après avoir posé l'owner). */
    #define SETUP() do{ econ->region[ra].owner=0; econ->region[rb].owner=1; \
        econ->region[ra].price[g]=1.0f; econ->region[rb].price[g]=6.0f; \
        set_rstock(econ,ra,g,500.f); set_rgold(econ,ra,0.f); \
        set_rstock(econ,rb,g,0.f);   set_rgold(econ,rb,100000.f); \
    ...
    printf("   après le tick : B reçoit %.0f unités · A encaisse %.0f or · valeur échangée %.0f\n",
           rstock(econ,rb,g), rgold(econ,ra), intertrade_imports_value(econ));
    ok("le bien REMONTE la pente de prix (l'importateur B reçoit)", rstock(econ,rb,g) > 0.f);
    ok("l'EXPORTATEUR A encaisse de l'or", rgold(econ,ra) > 0.0);
    ...
    intertrade_tick(econ,&rn,&dp);
    ok("EMBARGO : aucun bien ne passe entre pays EN GUERRE", rstock(econ,rb,g)==0.f);
    ...
    ok("après la paix, le commerce REPREND (B reçoit de nouveau)", rstock(econ,rb,g) > 0.f);

    /* ---- 4. Intra-pays ≠ commerce inter-pays ---- */
    printf("\n── 4. Une route intra-pays n'est pas du commerce inter-pays ──\n");
    /* STOCK NATIONAL : sous la MÊME couronne, les deux bouts puisent déjà au même entrepôt —
     * il n'y a plus de « stock de rb » à regarder. L'invariant testé est le MÊME : le tick
     * ne fait RIEN passer (l'entrepôt du pays 0 ne bouge pas, le pays 1 ne reçoit rien). */
    SETUP();
    econ->region[rb].owner=0; mirror_prov(econ,rb);   /* même couronne aux deux bouts */
    econ->nat_stock[0][g]=500.f; econ->nat_stock[1][g]=0.f;
    { float a0=econ_country_stock_sum(econ,0,(Resource)g);
      intertrade_tick(econ,&rn,&dp);
      ok("même couronne aux deux bouts → pas de route INTER-pays (rien ne passe ici)",
         fabsf(econ_country_stock_sum(econ,0,(Resource)g)-a0)<1e-3f
         && econ_country_stock_sum(econ,1,(Resource)g)==0.f); }
    ...
    /* pr = une région NON-Centre voisine de ra (son hub sera ra, distance 1) : elle est au
     * JOUEUR (pays 0), le Centre ra reste ÉTRANGER (pays 1).
     * STOCK/TRÉSOR NATIONAUX (2026-09-03) : la fixture d'origine donnait pr ET le Centre ra
     * au MÊME pays — acheter à son propre entrepôt est désormais un no-op EXACT (le bien est
     * pris à nat_stock[0] et rendu à nat_stock[0], l'or débité puis recrédité au même pays),
     * si bien qu'aucune des quatre assertions ne mesurait plus le verbe. Le marché est donc
     * celui d'un AUTRE pays — c'est là que « acheter au marché » veut dire quelque chose. */
    econ->region[ra].owner=1; mirror_prov(econ,ra);
    int pr=-1;
    ...
    if (pr<0){ printf("   (pas de voisin non-Centre pour ra — test sauté)\n"); }
        ...
        mirror_prov(econ,pr); mirror_prov(econ,ra);     /* l'appartenance descend à prov[] */
        econ->region[pr].price[RES_WOOD]=1.0f;
        set_rstock(econ,pr,RES_WOOD,0.f);              /* le joueur n'a RIEN */
        set_rgold (econ,pr,100000.f);
        set_rstock(econ,ra,RES_WOOD,500.f);            /* le marché (Centre ÉTRANGER ra) EN a */
        ...
        double tres0=rgold(econ,pr); float hub0=rstock(econ,ra,RES_WOOD);
        ...
        ok("le joueur REÇOIT son bois (50)", got==50 && rstock(econ,pr,RES_WOOD)==50.f);
        ok("il a PAYÉ au prix courant×marge (le pump du trésor)",
           spent==(long)(50*1.0f*marge+0.5f) && rgold(econ,pr) < tres0);
        ok("le bien VIENT du marché (le Centre ÉTRANGER ra se DÉPLÉTÉ de 50)",
           rstock(econ,ra,RES_WOOD)==hub0-50.f);
        ...
        set_rstock(econ,ra,RES_WOOD,0.f);
        double tres1=rgold(econ,pr);
        ...
        ok("marché VIDE → aucun achat (« uniquement s'il est dispo »), trésor intact",
           got2==0 && spent==0 && rgold(econ,pr)==tres1);
    }
    ...
    econ->region[ra].owner=0; mirror_prov(econ,ra);
    econ->region[ra].price[RES_WOOD]=2.f;
    set_rstock(econ,ra,RES_WOOD,200.f);
    set_rgold (econ,ra,5000.f);
    intertrade_tick(econ,&rn,&dp);                 /* (re)bâtit la carte : ra est son propre hub */
    econ_aggregate_regions(econ);
    float s0=rstock(econ,ra,RES_WOOD); double t0=rgold(econ,ra);
    long xp=0;
    ...
    ok("V2 : buy/sell same-region REFUSÉS (0) et NI stock NI trésor ne bougent (or infini TUÉ)",
       xb==0 && xs==0 && rstock(econ,ra,RES_WOOD)==s0 && rgold(econ,ra)==t0);

    /* ---- 7. NON-RÉGRESSION : le TRIPTYQUE empire-aware (gate = devis = consume) ----
     *   La matière de l'empire est mise en commun (depuis 2026-09-03 : LITTÉRALEMENT, un
     *   entrepôt national) : le gate la VOIT, le devis ne la FACTURE PAS (gratuite, marge 0),
     *   la conso la PUISE — et exactement d'autant. Les trois doivent s'accorder, sinon le
     *   joueur est sur/sous-facturé (le bug que ce banc verrouille). */
    printf("\n── 7. Empire-aware : la matière de l'empire est GRATUITE (gate=devis=consume) ──\n");
    {
        /* X = le chantier, Y = une région SŒUR du même empire 0. VAGUE ÎLES : les deux
         * premières régions du monde peuvent être des îlots SANS AUCUNE province active
         * (region_rep_prov=-1) — une région sans porteuse n'a pas de propriétaire stable,
         * donc plus ni caisse ni entrepôt (STOCK NATIONAL, cas limite tranché du design).
         * On choisit donc X et Y parmi les régions dont la REPRÉSENTATIVE est une vraie
         * province ACTIVE — le même repère que le moteur utilise pour résoudre l'owner. */
        int X=-1, Y=-1;
        for (int r=0;r<econ->n_regions;r++){
        ...
        econ_aggregate_regions(econ);
        ok("la CONSO puise l'ENTREPÔT NATIONAL d'EXACTEMENT 100 (rien créé, rien détruit)",
           fabsf(econ_country_stock_sum(econ,0,RES_STONE)-(y0-100.f))<1e-2f);
        }
    }
    ...
    if (pr>=0) {
        ...
        mirror_prov(econ,ra); mirror_prov(econ,rb); mirror_prov(econ,pr);
        /* STOCK NATIONAL : aucun autre porteur de pierre au monde — un seul entrepôt à vider
         * par pays, puis on dote le SEUL Centre porteur (celui du pays ÉTRANGER 1). */
        for (int c=0;c<SCPS_MAX_COUNTRY;c++) econ->nat_stock[c][RES_STONE]=0.f;
        set_rstock(econ,ra,RES_STONE,200.f);               /* le SEUL Centre porteur — et il est ÉTRANGER */
        ...
        if (intertrade_region_hub(pr)==ra && econ->region[ra].owner!=econ->region[pr].owner){
            ...
            float ra0=rstock(econ,ra,RES_STONE);
            ...
            ok("la CONSO importe bien du Centre étranger ra (−100)",
               fabsf(rstock(econ,ra,RES_STONE)-(ra0-100.f))<1e-2f);
        } else {
    ...
    printf("\n── 9. Le prix du pool esclave respire avec sa profondeur ──\n");
    {
        ...
        e2->nat_treasury[0]=1e9f;   /* TRÉSOR NATIONAL : la caisse du vendeur est celle du pays */
        ...
        double treas0=econ_country_gold(e2,0);
        long sold_empty=intertrade_slave_sell(e2, 0, 100);
        float gained_empty=(sold_empty>0)?(float)((econ_country_gold(e2,0)-treas0)/(double)sold_empty):0.f;
        ...
        double treas1=econ_country_gold(e2,0);
        long sold_full=intertrade_slave_sell(e2, 0, 100);
        float gained_full=(sold_full>0)?(float)((econ_country_gold(e2,0)-treas1)/(double)sold_full):0.f;
        ...
        double treas2=econ_country_gold(e2,0);
        long bought_rare=intertrade_slave_buy(e2, 0, 10, true, -1);
        buy_empty_cost=(bought_rare>0)?(float)((treas2-econ_country_gold(e2,0))/(double)bought_rare):0.f;
        intertrade_reset();
        e2->nat_treasury[0]=1e9f;
        ...
        double treas3=econ_country_gold(e2,0);
        long bought_full=intertrade_slave_buy(e2, 0, 10, true, -1);
        float buy_full_cost=(bought_full>0)?(float)((treas3-econ_country_gold(e2,0))/(double)bought_full):0.f;
