...
[T_ROBUSTE]      = { "Robuste",      CAT_PHYSIQUE, +2, T_FRELE,
    ...
    "Forte constitution : +1 Coercition (militaire), +20 % de Productivité." },
[T_REGENERANT]   = { "Régénérant",   CAT_PHYSIQUE, +2, T_CONVALESCENT,
    { .demographie=+0.20f, .coercition=+1.0f },
    "Sang de troll, guérit vite : +20 % de Démographie, +1 Coercition (militaire)." },
[T_PROLIFIQUE]   = { "Prolifique",   CAT_PHYSIQUE, +1, T_LENT_CROITRE,
    { .demographie=+0.15f },
    "Reproduction rapide : +15 % de Démographie." },
[T_LONGEVIF]     = { "Longévif",     CAT_PHYSIQUE, +1, T_EPHEMERE,
    { .derive=-0.20f },
    "Vie longue : −20 % de Dérive culturelle (héritage stable, dirigeants durables)." },
[T_ENDURANT]     = { "Endurant",     CAT_PHYSIQUE, +1, T_FRAGILE_CLIMAT,
    { .demographie=+0.10f },
    "Résiste au climat et à la disette : +10 % de Démographie." },
[T_SOBRE]        = { "Sobre",        CAT_PHYSIQUE, +1, T_VORACE,
    { .rendement=+0.10f },
    "Consomme peu : +10 % de Productivité (entretien faible)." },
/* ---- PHYSIQUE — défauts ---- */
[T_FRELE]        = { "Frêle",        CAT_PHYSIQUE, -1, T_ROBUSTE,
    { .coercition=-1.0f, .rendement=-0.20f },
    "Faible constitution : −1 Coercition (militaire), −20 % de Productivité." },
[T_CONVALESCENT] = { "Convalescent", CAT_PHYSIQUE, -1, T_REGENERANT,
    { .demographie=-0.20f, .coercition=-1.0f },
    "Guérison lente : −20 % de Démographie, −1 Coercition (militaire)." },
[T_LENT_CROITRE] = { "Lent à croître",CAT_PHYSIQUE, -1, T_PROLIFIQUE,
    { .demographie=-0.15f },
    "Reproduction lente : −15 % de Démographie." },
[T_EPHEMERE]     = { "Éphémère",     CAT_PHYSIQUE, -1, T_LONGEVIF,
    { .derive=+0.20f },
    "Vie courte : +20 % de Dérive culturelle (l'héritage se dilue vite)." },
[T_FRAGILE_CLIMAT]={ "Fragile au climat",CAT_PHYSIQUE,-1, T_ENDURANT,
    { .demographie=-0.10f },
    "Souffre hors de sa niche : −10 % de Démographie." },
[T_VORACE]       = { "Vorace",       CAT_PHYSIQUE, -1, T_SOBRE,
    { .rendement=-0.10f },
    "Consomme beaucoup : −10 % de Productivité (entretien lourd)." },
/* ---- SOCIAL — atouts ---- */
[T_BELLIQUEUX]   = { "Belliqueux",   CAT_SOCIAL, +2, T_DEBONNAIRE,
    { .coercition=+1.0f },
    "Aime la guerre : +1 Coercition (militaire)." },
[T_SOUDE]        = { "Soudé",        CAT_SOCIAL, +2, T_FACTIEUX,
    { .fracture=-1.0f },
    "Solidarité large : −1 Fracture (la diversité tient)." },
[T_CHARISMATIQUE]= { "Charismatique",CAT_SOCIAL, +1, T_REBUTANT,
    { .influence=+0.5f },
    "Présence : +0.5 Influence (diplomatie, rayonnement)." },
[T_OUVERT]       = { "Ouvert",       CAT_SOCIAL, +1, T_INSULAIRE,
    { .permeabilite=+0.5f },
    "Hospitalier : +0.5 Perméabilité (assimile plus vite)." },
[T_DISCIPLINE]   = { "Discipliné",   CAT_SOCIAL, +1, T_FRONDEUR,
    { .capacite=+0.5f },
    "Ordre intériorisé : +0.5 Capacité (tient plus de diversité, moins d'agitation)." },
[T_PROSELYTE]    = { "Prosélyte",    CAT_SOCIAL, +1, T_RESERVE,
    { .influence=+0.25f, .derive=+0.20f },
    "Rayonne sa culture : +0.25 Influence, +20 % de Dérive culturelle." },
/* ---- SOCIAL — défauts ---- */
[T_DEBONNAIRE]   = { "Débonnaire",   CAT_SOCIAL, -1, T_BELLIQUEUX,
    { .coercition=-1.0f },
    "Rétif à la guerre : −1 Coercition (militaire)." },
[T_FACTIEUX]     = { "Factieux",     CAT_SOCIAL, -1, T_SOUDE,
    { .fracture=+1.0f },
    "Clans rivaux : +1 Fracture (la diversité se fend)." },
[T_REBUTANT]     = { "Rebutant",     CAT_SOCIAL, -1, T_CHARISMATIQUE,
    { .influence=-0.5f },
    "Déplaisant : −0.5 Influence." },
[T_INSULAIRE]    = { "Insulaire",    CAT_SOCIAL, -1, T_OUVERT,
    { .permeabilite=-0.5f },
    "Méfiant : −0.5 Perméabilité (assimile mal)." },
[T_FRONDEUR]     = { "Frondeur",     CAT_SOCIAL, -1, T_DISCIPLINE,
    { .capacite=-0.5f },
    "Indiscipliné : −0.5 Capacité (l'agitation tient mal)." },
[T_RESERVE]      = { "Réservé",      CAT_SOCIAL, -1, T_PROSELYTE,
    { .influence=-0.25f, .derive=-0.20f },
    "Sa culture ne s'exporte pas : −0.25 Influence, −20 % de Dérive culturelle." },
/* ---- INTELLECTUEL — atouts ---- */
[T_INVENTIF]     = { "Inventif",     CAT_INTELLECTUEL, +2, T_BORNE,
    { .rendement=+0.20f },
    "Innovation : +20 % de Productivité (travail et recherche)." },
[T_ARCANIQUE]    = { "Arcanique",    CAT_INTELLECTUEL, +2, T_SOURD_ARCANE,
    { .arcane=+1.0f },
    "Sensible à la magie : +1 Affinité arcane (la branche Magie s'ouvre, pente faustienne)." },
[T_STUDIEUX]     = { "Studieux",     CAT_INTELLECTUEL, +1, T_INCULTE,
    { .rendement=+0.10f },
    "Soif de savoir : +10 % de Productivité (travail et recherche)." },
[T_BATISSEUR]    = { "Bâtisseur",    CAT_INTELLECTUEL, +1, T_BROUILLON,
    { .capacite=+0.5f },
    "Méthodique : +0.5 Capacité (tient plus de diversité)." },
[T_ADAPTABLE]    = { "Adaptable",    CAT_INTELLECTUEL, +1, T_TRADITIONALISTE,
    { .derive=+0.20f },
    "Curieux : +20 % de Dérive culturelle (épouse le lieu, assimile vite)." },
[T_INDUSTRIEUX]  = { "Industrieux",  CAT_INTELLECTUEL, +1, T_INDOLENT,
    { .rendement=+0.10f },
    "Appliqué : +10 % de Productivité." },
/* ---- INTELLECTUEL — défauts ---- */
[T_BORNE]        = { "Borné",        CAT_INTELLECTUEL, -1, T_INVENTIF,
    { .rendement=-0.20f },
    "Esprit fermé : −20 % de Productivité (travail et recherche)." },
[T_SOURD_ARCANE] = { "Sourd à l'arcane",CAT_INTELLECTUEL,-1, T_ARCANIQUE,
    { .arcane=-1.0f },
    "Imperméable à la magie : −1 Affinité arcane (la branche Magie coûte très cher)." },
[T_INCULTE]      = { "Inculte",      CAT_INTELLECTUEL, -1, T_STUDIEUX,
    { .rendement=-0.10f },
    "Indifférent au savoir : −10 % de Productivité (travail et recherche)." },
[T_BROUILLON]    = { "Brouillon",    CAT_INTELLECTUEL, -1, T_BATISSEUR,
    { .capacite=-0.5f },
    "Désordonné : −0.5 Capacité." },
[T_TRADITIONALISTE]={ "Traditionaliste",CAT_INTELLECTUEL,-1, T_ADAPTABLE,
    { .derive=-0.20f },
    "Rigide : −20 % de Dérive culturelle (change mal, mais l'héritage est très stable)." },
[T_INDOLENT]     = { "Indolent",     CAT_INTELLECTUEL, -1, T_INDUSTRIEUX,
    { .rendement=-0.10f },
    "Nonchalant : −10 % de Productivité." },
};
