...
[T_ROBUSTE]      = { "Robuste",      CAT_PHYSIQUE, +2, T_FRELE,
    ...
    "Forte constitution : +0.5 Coercition (H, puissance militaire), +10 % de production & prospérité.",
...
[T_REGENERANT]   = { "Régénérant",   CAT_PHYSIQUE, +2, T_CONVALESCENT,
    ...
    "Sang de troll, guérit vite : +10 % de Démographie, +0.5 Coercition (H, puissance militaire).",
...
[T_PROLIFIQUE]   = { "Prolifique",   CAT_PHYSIQUE, +1, T_LENT_CROITRE,
    { .demographie=+0.10f },
    "Reproduction rapide : +10 % de Démographie.",
...
[T_LONGEVIF]     = { "Longévif",     CAT_PHYSIQUE, +1, T_EPHEMERE,
    ...
    "Vie longue : −20 % de Dérive culturelle (horloge & contact — l'héritage se garde), +0.25 Capacité (K, −7.5 % de pénalité de diversité).",
...
[T_ENDURANT]     = { "Endurant",     CAT_PHYSIQUE, +1, T_FRAGILE_CLIMAT,
    { .demographie=+0.10f },
    "Résiste aux épreuves : +10 % de Démographie.",
...
[T_SOBRE]        = { "Sobre",        CAT_PHYSIQUE, +1, T_VORACE,
    { .rendement=+0.10f },
    "Consomme peu : +10 % de production & prospérité.",
...
[T_FRELE]        = { "Frêle",        CAT_PHYSIQUE, -1, T_ROBUSTE,
    { .rendement=-0.10f },
    "Faible constitution : −10 % de production & prospérité.",
...
[T_CONVALESCENT] = { "Convalescent", CAT_PHYSIQUE, -1, T_REGENERANT,
    { .demographie=-0.10f },
    "Guérison lente : −10 % de Démographie.",
...
[T_LENT_CROITRE] = { "Lent à croître",CAT_PHYSIQUE, -1, T_PROLIFIQUE,
    { .demographie=-0.10f },
    "Reproduction lente : −10 % de Démographie.",
...
[T_EPHEMERE]     = { "Éphémère",     CAT_PHYSIQUE, -1, T_LONGEVIF,
    { .derive=+0.20f, .capacite=-0.25f },
    "Vie courte : +20 % de Dérive culturelle (l'héritage se dilue au contact), −0.25 Capacité (K, +7.5 % de pénalité de diversité).",
...
[T_FRAGILE_CLIMAT]={ "Fragile au climat",CAT_PHYSIQUE,-1, T_ENDURANT,
    { .demographie=-0.10f },
    "Constitution fragile : −10 % de Démographie.",
...
[T_VORACE]       = { "Vorace",       CAT_PHYSIQUE, -1, T_SOBRE,
    { .rendement=-0.10f },
    "Consomme beaucoup : −10 % de production & prospérité.",
...
[T_BELLIQUEUX]   = { "Belliqueux",   CAT_SOCIAL, +2, T_DEBONNAIRE,
    { .coercition=+1.0f },
    "Aime la guerre : +1 Coercition (H, puissance militaire).",
...
[T_SOUDE]        = { "Soudé",        CAT_SOCIAL, +2, T_FACTIEUX,
    { .fracture=-1.0f },
    "Solidarité large : −1 Fracture (fracture interne D̄, −0.06 sur le grief de révolte).",
...
[T_CHARISMATIQUE]= { "Charismatique",CAT_SOCIAL, +1, T_REBUTANT,
    { .influence=+0.5f },
    "Présence : +5 d'Influence d'État (standing, portée diplomatique).",
...
[T_OUVERT]       = { "Ouvert",       CAT_SOCIAL, +1, T_INSULAIRE,
    { .permeabilite=+0.5f },
    "Hospitalier : +0.5 Perméabilité (P — assimile les minorités plus vite).",
...
[T_DISCIPLINE]   = { "Discipliné",   CAT_SOCIAL, +1, T_FRONDEUR,
    { .capacite=+0.5f },
    "Ordre intériorisé : +0.5 Capacité (K) ; les minorités sont mieux servies (−15 % de pénalité de diversité).",
...
[T_PROSELYTE]    = { "Prosélyte",    CAT_SOCIAL, +1, T_RESERVE,
    ...
    "Rayonne sa culture : +2.5 d'Influence d'État, +0.25 Perméabilité (P — assimile mieux les minorités qu'il rencontre).",
...
[T_DEBONNAIRE]   = { "Débonnaire",   CAT_SOCIAL, -1, T_BELLIQUEUX,
    { .coercition=-0.5f },
    "Rétif à la guerre : −0.5 Coercition (H, puissance militaire).",
...
[T_FACTIEUX]     = { "Factieux",     CAT_SOCIAL, -1, T_SOUDE,
    { .fracture=+0.5f },
    "Clans rivaux : +0.5 Fracture (fracture interne D̄, +0.03 sur le grief de révolte).",
...
[T_REBUTANT]     = { "Rebutant",     CAT_SOCIAL, -1, T_CHARISMATIQUE,
    { .influence=-0.5f },
    "Déplaisant : −5 d'Influence d'État.",
...
[T_INSULAIRE]    = { "Insulaire",    CAT_SOCIAL, -1, T_OUVERT,
    { .permeabilite=-0.5f },
    "Méfiant : −0.5 Perméabilité (assimile les minorités plus lentement).",
...
[T_FRONDEUR]     = { "Frondeur",     CAT_SOCIAL, -1, T_DISCIPLINE,
    { .capacite=-0.5f },
    "Indiscipliné : −0.5 Capacité (K) ; les minorités sont moins bien servies (+15 % de pénalité de diversité).",
...
[T_RESERVE]      = { "Réservé",      CAT_SOCIAL, -1, T_PROSELYTE,
    { .influence=-0.25f, .permeabilite=-0.25f },
    "Sa culture ne s'exporte pas : −2.5 d'Influence d'État, −0.25 Perméabilité (P — assimile moins vite les minorités qu'il rencontre).",
...
[T_INVENTIF]     = { "Inventif",     CAT_INTELLECTUEL, +2, T_BORNE,
    { .rendement=+0.20f },
    "Innovation : +20 % de production & prospérité.",
...
[T_ARCANIQUE]    = { "Arcanique",    CAT_INTELLECTUEL, +2, T_SOURD_ARCANE,
    { .arcane=+1.0f },
    "Sensible à la magie : la branche faustienne coûte −25 % (nœuds), mais la pente vers la Brèche s'accentue (+1 de flux).",
...
[T_STUDIEUX]     = { "Studieux",     CAT_INTELLECTUEL, +1, T_INCULTE,
    { .capacite=+0.5f },
    "Soif de savoir : +0.5 Capacité (K) ; les minorités sont mieux servies (−15 % de pénalité de diversité).",
...
[T_BATISSEUR]    = { "Bâtisseur",    CAT_INTELLECTUEL, +1, T_BROUILLON,
    ...
    "Méthodique : +0.25 Capacité (K, organisation, −7.5 % de pénalité de diversité), +5 % de production & prospérité.",
...
[T_ADAPTABLE]    = { "Adaptable",    CAT_INTELLECTUEL, +1, T_TRADITIONALISTE,
    { .permeabilite=+0.5f },
    "Curieux : +0.5 Perméabilité (P — assimile les minorités qu'il rencontre).",
...
[T_INDUSTRIEUX]  = { "Industrieux",  CAT_INTELLECTUEL, +1, T_INDOLENT,
    { .rendement=+0.10f },
    "Appliqué : +10 % de production & prospérité.",
...
[T_BORNE]        = { "Borné",        CAT_INTELLECTUEL, -1, T_INVENTIF,
    { .rendement=-0.10f },
    "Esprit fermé : −10 % de production & prospérité.",
...
[T_SOURD_ARCANE] = { "Sourd à l'arcane",CAT_INTELLECTUEL,-1, T_ARCANIQUE,
    { .arcane=-1.0f },
    "Imperméable à la magie : la branche faustienne coûte +25 % (nœuds, protection), mais −1 de flux lui ferme aussi les débouchés arcanes.",
...
[T_INCULTE]      = { "Inculte",      CAT_INTELLECTUEL, -1, T_STUDIEUX,
    { .capacite=-0.5f },
    "Indifférent au savoir : −0.5 Capacité (K) ; les minorités sont moins bien servies (+15 % de pénalité de diversité).",
...
[T_BROUILLON]    = { "Brouillon",    CAT_INTELLECTUEL, -1, T_BATISSEUR,
    { .capacite=-0.25f, .rendement=-0.05f },
    "Désordonné : −0.25 Capacité (K, +7.5 % de pénalité de diversité), −5 % de production & prospérité.",
...
[T_TRADITIONALISTE]={ "Traditionaliste",CAT_INTELLECTUEL,-1, T_ADAPTABLE,
    ...
    "Rigide : −20 % de Dérive culturelle (change mal — l'héritage est très stable), −0.5 Perméabilité (P — assimile mal les minorités qu'il rencontre).",
...
[T_INDOLENT]     = { "Indolent",     CAT_INTELLECTUEL, -1, T_INDUSTRIEUX,
    { .rendement=-0.10f },
    "Nonchalant : −10 % de production & prospérité.",
