...
int main(int argc, char **argv){
    ...
    /* ── 1a. Élites CONNUES (prov[], jamais region[].pop) ── */
    printf("\n-- 1. Generation mensuelle : PER_NOBLE x elites(prov[]) x mult_conseil --\n");
    double elites_before=0.0;
    for (int i=0;i<econ->n_prov;i++)
        if (econ->prov[i].owner==cid){ elites_before += econ->prov[i].strata[CLASS_ELITE].pop; econ->prov[i].strata[CLASS_ELITE].pop=0.f; }
    int fixed_prov=-1;
    ...
    econ->prov[fixed_prov].strata[CLASS_ELITE].pop = 1000.f;   /* élites(cid) = 1000, EXACT */
    double elites_now = influence_elites(econ, cid);
    ok("influence_elites somme les PROVINCES au pays (prov[], jamais region[].pop stale)",
